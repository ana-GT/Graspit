...
*/
class SimAnnPlus : public QObject
{
	Q_OBJECT
public:
...
private:
	...
	void variablePosition( VariableSet* _p, double _T,
			       VariableSet* _q );
	void variableQuaternion(  SearchVariable *_w,
				  SearchVariable *_x,
				  SearchVariable *_y,
				  SearchVariable *_z,
				  double _T );
	Quaternion uniformQuaternion();
	void variableOne( SearchVariable *_var, double _T );
	double neighborDistribution(double T);
	double neighborDistribution2(double T);
